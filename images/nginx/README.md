...
2. Download: `docker pull swatkiev/nginx-stream`

### Usage

Start deamon
```bash
$ docker run -d -p 0.0.0.0:80:80 --name nginx swatkiev/nginx-stream
```
...
```bash
$ docker run -d -p 80:80 -p 65432:65432 -v `pwd`\http.conf.d:/opt/nginx/http.conf.d  -v `pwd`\stream.conf.d:/opt/nginx/stream.conf.d --name nginx swatkiev/nginx-stream
```
